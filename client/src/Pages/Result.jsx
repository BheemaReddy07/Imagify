    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col min-h-[90vh] items-center justify-center '>
      <div  >
        <div className='relative'>
          ...
          <span className={`absolute bg-blue-500  bottom-0 left-0 h-1 ${loading?'transition-all  w-full duration-[10s]':'w-0'}`}  /> 
         </div>
      ...
      <div className='flex w-full max-w-xl bg-neutral-500 text-white text-sm p-0.5 mt-10 rounded-full '>
        ...
        <button type='submit' className='bg-zinc-900 px-10 sm:px-16 py-3 rounded-full' onChange={e => setInput(e.target.value)} value={input}>Generate</button>
      </div> }
      ...
      <div className='flex gap-2 flex-wrap justify-center text-white text-sm p-0.5 mt-10 rounded-full'>
          ...
          <a href={image} download   className='bg-zinc-900 px-10 py-3 text-white rounded-full cursor-pointer'> Download</a>
      </div>
      }
     </form>
  )
